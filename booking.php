                ...
                if (!$vehicle) {
                    ...
                    if (isAvailable($pdo, $vehicle_id, $start_date, $end_date)) {
                        ...
                        ");
                            ...
                            $pdo->commit();
                            header("Location: facture.php?id=" . $reservation_id);
                            exit;
                        } catch (Exception $e) {
                            $pdo->rollBack();
                ...
                <form method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <small><a href="#" class="text-decoration-none text-primary" data-bs-toggle="modal"
                                    data-bs-target="#addClientModal">+ Add New Client</a></small>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="start_date" class="form-control" required
                                min="<?= date('Y-m-d') ?>" value="<?= htmlspecialchars($start_date ?? '') ?>">
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="damage_log" class="form-control" rows="3"
                            placeholder="Note existing scratches, fuel level, etc..."><?= htmlspecialchars($damage_log ?? '') ?></textarea>
                    </div>

                    <div class="d-flex justify-content-between">
                        <a href="index.php" class="btn action-btn ghost">Cancel</a>
                        <button type="submit" class="btn action-btn"><i class="fas fa-check-circle me-2"></i> Confirm
                            Booking</button>
                    </div>
                </form>
            </section>
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="full_name" class="form-control" required
                                value="<?= htmlspecialchars($new_client_name) ?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Phone</label>
                            <input type="text" name="phone" class="form-control" required
                                value="<?= htmlspecialchars($new_client_phone) ?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">License Number</label>
                            <input type="text" name="license_number" class="form-control" required
                                value="<?= htmlspecialchars($new_client_license) ?>">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Address</label>
                            <textarea name="address" class="form-control"
                                rows="3"><?= htmlspecialchars($new_client_address) ?></textarea>
                        </div>
                        <div class="d-flex justify-content-between">
                            <button type="button" class="btn action-btn ghost" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn action-btn"><i class="fas fa-user-plus me-2"></i>Add
                                Client</button>
                        </div>
                    </form>
                </div>
